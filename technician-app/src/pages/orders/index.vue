...
export default {
  ...
  onLoad(query) {
    if (query.status) this.activeTab = query.status
    else { var f=uni.getStorageSync('orderFilter'); if(f){this.activeTab=f;uni.removeStorageSync('orderFilter')} }
    // 监听工单更新事件
    uni.$on('orderUpdated', () => { this.loadOrders(true) })
  },
  onUnload() { uni.$off('orderUpdated') },
  onShow() {
...
.load-more-wrap{text-align:center;padding:var(--spacing-lg) 0;margin-bottom:var(--spacing-lg)}
.load-more-btn{font-size:var(--font-sm);color:var(--primary);font-weight:500;padding:8px 20px;background:var(--bg-card);border-radius:var(--radius-round);border:1px solid var(--border)}
.load-end-text{font-size:var(--font-sm);color:var(--text-tertiary)}
</style>
